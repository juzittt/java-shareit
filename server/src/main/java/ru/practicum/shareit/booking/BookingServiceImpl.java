...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingDto> getBookings(Long userId, String state) {
        log.info("[getBookings] Getting list bookings. userId={}, state={}", userId, state);
        validateUser(userId);
        ...
        return toDtoList(
                switch (state.toUpperCase()) {
                    ...
                    case ("CURRENT") ->
                            bookingRepository.findCurrentByBookerId(userId, LocalDateTime.now());
                    case ("PAST") ->
                            bookingRepository.findPastByBookerId(userId, LocalDateTime.now());
                    case ("FUTURE") ->
                            bookingRepository.findFutureByBookerId(userId, LocalDateTime.now());
                    case ("WAITING") ->
